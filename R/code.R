...
plotGeneModel <- function(gene,uORF,Extend=Extend,p.isoform=isoform,uORF.isoform=NULL){
  ...
  for (i in sort(unlist(txByGene[gene])$tx_name)) {
    ...
    if (i %in% names(threeUTR)) {
      ...
      if (p.isoform==k){
        text(x=min(start(generanges))-Extend-20, y=(yAxis-0.28*k2+0.05), labels=tx_num[k2],cex=1.4,font=2)
      } else {
        text(x=min(start(generanges))-Extend-20, y=(yAxis-0.28*k2+0.05), labels=tx_num[k2],cex=1.2)
      }
    }
    else {
